...
# This script uses the template healthcare-connection-profile.json to generate a connection profile
# for the organisations in the Fabric network.

#REPODIR points to this repo.
REPODIR=~/Documents/apps/Healthcare-Supplychain-Blockchain
...
cp healthcare-connection-profile.yaml $REPODIR/tmp/connection-profile
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -e "s|%REPODIR%|$CERTDIR|g" healthcare-connection-profile.yaml > $REPODIR/tmp/connection-profile/healthcare-connection-profile.yaml
else
    sed -i "s|%REPODIR%|$CERTDIR|g"  $REPODIR/tmp/connection-profile/healthcare-connection-profile.yaml
fi 
