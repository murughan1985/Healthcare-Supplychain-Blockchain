...
echo Executing command: aws managedblockchain create-network --region $REGION \
    ...
    --name "${NETWORKNAME}" \
    --description "Healthcare Fabric network"
    ...
    --member-configuration "Name=\"${MEMBERNAME}\",Description=\"Healthcare Fabric member\",FrameworkConfiguration={Fabric={AdminUsername=${ADMINUSER},AdminPassword=${ADMINPWD}}}"
